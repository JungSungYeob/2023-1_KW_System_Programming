...
// ---------------------------------------------------------------------//
// Title : System Programming Assignment #2-3 (advanced Web Server)		//
...
//              alarm 10 seconds for print connetion history            //
...
void sub_Queue(){
    ...
    for(int i=0;i<9;i++){ //delete first queue
...
void add_Queue(struct in_addr IP, int PID, int PORT, time_t TIME){
    if(is_full()){ //if full delete first queue
        sub_Queue();
    }
    //adding queue
...
int compare_WhiteList(char* IP_Address){
    ...
    while (fgets(line, sizeof(line),file)!=NULL){//get line of accessible.usr
        unsigned int len = strlen(line);
        if(len>0&&line[len-1]=='\n'){ //change '\n' to '\0'
            line[len-1]= '\0';
        }
        if(fnmatch(line,IP_Address,0)==0){ //fnmatch line and IP_Address
            return 0;
        }else{
            continue;
        }
    ...
    for (int i = 0; i < num_cnt - 1; i++) {
        for (int j = 0; j < num_cnt - i - 1; j++) {
            ...
            if (strcasecmp(cmp_tmp1, cmp_tmp2) >
                0) { //compare name, if com_tmp1 is bigger than cmp_tmp2, change location
...
//	                                                                    //
//////////////////////////////////////////////////////////////////////////
void create_ls(int argc, char *argv[], int flag_a, int flag_l, char url[], int client_fd,FILE *file,char* filename) {
    DIR *dirp;//DIR pointer dirp
    struct dirent *dir;
...
int main(int argc, char *argv[]) {//for server operation, not for create html file.
    ...
    while (1) {
        ...
        add_Queue(inet_client_address,pid,client_addr.sin_port,executed_time);//add new client to queue
        ...
        if (pid == -1) { // pid failed
            ...
            if(compare_WhiteList(inet_ntoa(client_addr.sin_addr))!=0){ // not accessible
                sprintf(response_message,
                            "<h1>Access denied</h1>"
                            "<h3>Your IP: %s</h3>"
                            "You have no permission to access this web server.<br>"
                            "HTTP 403.6 - Forbidden: IP address reject",inet_ntoa(inet_client_address));
            ...
            if (strcmp(url, "/") == 0) {
                create_ls(argc, argv, 0, 1, url, client_fd,file,filename); //if directory it makes html file
            } else {
                while (url[strlen(url) - 1] == '/') {//delete last '/' for generalization
                    url[strlen(url) - 1] = '\0';
                }
                if (url[0] == '\0') {
                    create_ls(argc, argv, 0, 1, url, client_fd,file,filename);
                } else {
                    create_ls(argc, argv, 1, 1, url, client_fd,file,filename);//if directory it makes html file
                }
            }
            fclose(file);//close file
            ...
            if ((dirp = opendir(path)) == NULL) {//if not directory
                ...
                if ((checkFile = open(path, O_RDONLY)) == -1) {//if not existing directory
                    ...
                    if (fnmatch("*.jpg", path, FNM_CASEFOLD) == 0 || fnmatch("*.jpeg", path, FNM_CASEFOLD) == 0 ||
                        fnmatch("*.png", path, FNM_CASEFOLD) == 0) {//if image file
                        // get file size
                        off_t offset = 0;
                        struct stat file_stat;
                        ...
                        if (S_ISLNK(file_stat.st_mode) != 0) { //if link file
                            ...
                            if ((linkFile = open(linkpath, O_RDONLY)) == -1) {//if path is error
                                ...
                                filesize = link_stat.st_size;
                                if (fnmatch("*.jpg", linkpath, FNM_CASEFOLD) == 0 ||
                                    fnmatch("*.jpeg", linkpath, FNM_CASEFOLD) == 0 ||
                                    fnmatch("*.png", linkpath, FNM_CASEFOLD) == 0) {//if image file
                                    sprintf(response_header, "HTTP/1.0 200 OK\r\n"
                                                             "Server: 2019202021 advanced web server\r\n"
                                                             "Content-length:%lu\r\n"
                                                             "Content-type:image/*\r\n\r\n", filesize);
