/////////////////////////////////////////////////////////////////////////
// File Name   : 2019202021_ipc_server.c							//
// Date        : 2023/5/24												//
...
{
    History *shm_addr;
    if((shm_addr=shmat(shm_id, NULL, 0))==(void*)-1){ //get shared memory address
...
{ // for parent
    ...
    History *shm_addr;
    if((shm_addr=shmat(shm_id, NULL, 0))==(void*)-1){ //get shared memory
        printf("shmat fail -main\n");
        exit(1);
    }
...
void idle_handler2(int sig){
    ...
    strftime(time_str, sizeof(time_str), "%a %b %d %H:%M:%S %Y", time_tm);
    printf("[%s] IdleProcessCount : %d\n",time_str,++process_cnt);
    if(process_cnt>systemConf.MaxIdleNum){ // if over than maxindlenum
        while(process_cnt!=5){
...
{ // for server operation, not for create html file.
    ...
    History *shm_addr;
    if((shm_id = shmget((key_t)KEY_NUM,maxhistorySize,IPC_CREAT|0666))==-1){ //get id by key 40000
    ...
    while(ending_node!=NULL){ //exit simple handler
        wait(&status);
    ...
    if(shmdt(shm_addr)==-1){ //distribute shared memory
